...
NSString * const storedImpDisplayInterstitialAdMob = @"imp-prebid-display-interstitial-320-480";
NSString * const storedResponseDisplayInterstitialAdMob = @"response-prebid-display-interstitial-320-480";
...
- (void)loadView {
    ...
    Prebid.shared.storedAuctionResponse = storedResponseDisplayInterstitialAdMob;
    [self createAd];
}
...
- (void)createAd {
    ...
    [self.admobAdUnit fetchDemandWithCompletion:^(enum ResultCode resultCode) {
        @weakify(self);
        [GADInterstitialAd loadWithAdUnitID:adMobAdUnitDisplayInterstitial request:self.gadRequest completionHandler:^(GADInterstitialAd * _Nullable interstitialAd, NSError * _Nullable error) {
            @strongify(self);
            if (error != nil) {
