...
public final class VideoUtils: NSObject {
    ...
    public static func buildAdTagUrl(adUnitId: String, targeting: Dictionary<String, String>) -> String {
        ...
        //TODO
        let adTagUrl = buildAdTagUrl(adUnitId: adUnitId, adSize: targeting["hb_size_rubicon"], customParams: customParamsString!)
        
        return adTagUrl
    }
    
    //https://support.google.com/admanager/answer/1068325?hl=en
    public static func buildAdTagUrl(adUnitId: String, adSize:String?, customParams: String?) -> String {
...
//                "&impl=s" +
        ...
        if let adSize = adSize {
            result += "&sz=\(adSize)" //Size of master video ad slot. Multiple sizes should be separated by the pipe (|) character.
        }
        
        if let customParams = customParams {
            result += "&cust_params=\(customParams)"
        }
