...
@interface OXMViewExposureChecker()
...
@implementation OXMViewExposureChecker

// MARK: - Public API

+ (OXMViewExposure *)exposureOfView:(UIView *)view {
    return [[[OXMViewExposureChecker alloc] initWithView:view] exposure];
}
...
#   ifdef DEBUG
    if ([OXASDKConfiguration singleton].forcedIsViewable) {
        return [[OXMViewExposure alloc] initWithExposureFactor:1 visibleRectangle:self.testedView.bounds occlusionRectangles:nil];
    }
#   endif
    
    if (!self.testedView || self.testedView.isHidden || self.testedView.superview == nil || ![self isOnForeground]) {
        return [OXMViewExposure zeroExposure];
    }
    ...
    return [[OXMViewExposure alloc] initWithExposureFactor:(clipArea - obstructedArea)/fullArea
