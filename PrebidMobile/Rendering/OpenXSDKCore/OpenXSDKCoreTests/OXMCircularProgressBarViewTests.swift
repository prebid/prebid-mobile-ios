//
//  OXMCircularProgressBarViewTests.swift
//  OpenXSDKCoreTests
//
...
class OXMCircularProgressBarViewTests: XCTestCase {

    func testInit() {        
        var circularProgressBarView = OXMCircularProgressBarView()
        XCTAssertNotNil(circularProgressBarView)

        let viewRect = CGRect(x: 0, y: 0, width: 50, height: 50)
        circularProgressBarView = OXMCircularProgressBarView(frame: viewRect)
        XCTAssertNotNil(circularProgressBarView)
    }
    ...
    func testProperties() {
        ...
        let circularProgressBarView = OXMCircularProgressBarView()
        XCTAssertNotNil(circularProgressBarView)
    ...
    func testDataObject() {
        let data = NSKeyedArchiver.archivedData(withRootObject: OXMCircularProgressBarView())
        XCTAssertNotNil(data)
        let object = NSKeyedUnarchiver.unarchiveObject(with: data)
        XCTAssert(object is OXMCircularProgressBarView)
    }
}
