...
class OXANativeAdMarkupTest: XCTestCase {
    func testInitFromJson() {
        ...
        let requiredProperties: [(JSONDecoding.PropertyCheck<OXANativeAdMarkup>, Error)] = []
        ...
        let eventTrackerDic: [String: Any] = [
            "event": OXANativeEventType.impression.rawValue,
            "method": OXANativeEventTrackingMethod.img.rawValue,
            "url": "event tracker url",
            "customdata": ["g": "h"],
            "ext": ["eta": "etb"],
        ]
        
        let optionalLinkProperties: [JSONDecoding.BaseOptionalCheck<OXANativeAdMarkup>] = [
            ...
            JSONDecoding.OptionalPropertyCheck(value: [try! OXANativeAdMarkupAsset(jsonDictionary: assetDic)],
        ...
        let markupTester = JSONDecoding.Tester(generator: OXANativeAdMarkup.init(jsonDictionary:),
    ...
    func testIsEqual() {
        let tester: Equality.Tester<OXANativeAdMarkup> =
            Equality.Tester(template: OXANativeAdMarkup(link: .init(url: "")), checks: [
                Equality.Check(values: "0.4", "1.2", keyPath: \.version),
                Equality.Check(values: [
                    OXANativeAdMarkupAsset(data: .init(value: "data value")),
                ], [
                    OXANativeAdMarkupAsset(video: .init(vastTag: "vast XML !")),
                ], keyPath: \.assets),
                ...
                Equality.Check(values: [
                    OXANativeAdMarkupEventTracker(event: .MRC50, method: .JS, url: "event url 1"),
                ], [
                    OXANativeAdMarkupEventTracker(event: .impression, method: .img, url: "event-2 url"),
    ...
    func testErrorInvalidJsonString() {
        ...
        for (jsonString, errorPrefix) in invalidities {
            do {
                _ = try OXANativeAdMarkup(jsonString: jsonString)
                XCTFail()
            } catch {
    ...
    func testInitFromJsonString() {
        ...
        XCTAssertEqual(try! OXANativeAdMarkup(jsonString: jsonString), try! OXANativeAdMarkup(jsonDictionary: jsonDic))
    }
}
