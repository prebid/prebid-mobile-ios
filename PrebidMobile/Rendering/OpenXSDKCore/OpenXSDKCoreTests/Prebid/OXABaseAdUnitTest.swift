...
class OXABaseAdUnitTest: XCTestCase, WinningBidResponseFabricator {
    func testDesignatedInit_noBlockCalled() {
        ...
        let adUnit = OXABaseAdUnit(configID: configID) { adUnitConfig in
            XCTFail()
    ...
    func testFetchDemand_DemandError() {
        ...
        let adUnit = OXABaseAdUnit(configID: configID) { adUnitConfig in
            ...
            return MockBidRequester(expectedCalls: [
                { responseHandler in
                    responseHandler(nil, OXAError.invalidAccountId)
                },
            ])
        } winNotifierBlock: { (bid, adMarkupStringHandler) in
            XCTFail()
    ...
    func testFetchDemand_NoBids() {
        ...
        let bidResponse = OXABidResponse(rawBidResponse: .init())
