...
class OXAWinNotifierTest: XCTestCase {
...
"""
    ...
    func testCacheUrlFromTargeting() {
        XCTAssertEqual(OXAWinNotifier.cacheUrl(fromTargeting: normalTargeting, idKey: "hb_uuid"), cacheUrl)
        XCTAssertEqual(OXAWinNotifier.cacheUrl(fromTargeting: normalTargetingWithCachedId, idKey: "hb_cache_id"), cacheUrl)
        for key in normalTargeting.keys {
            var incompleteTargeting = normalTargeting
            incompleteTargeting[key] = nil
            XCTAssertNil(OXAWinNotifier.cacheUrl(fromTargeting: incompleteTargeting, idKey: "hb_uuid"))
        }
    }
                                  ...
                                  line: UInt = #line)
    {
        ...
        let connection = MockServerConnection(onDownload: expectedUrls.map { expectedDestination in { url, callback in
            ...
            let response = OXMServerResponse()
        ...
        OXAWinNotifier.notifyThroughConnection(connection, winning: bid) { resultMarkup in
