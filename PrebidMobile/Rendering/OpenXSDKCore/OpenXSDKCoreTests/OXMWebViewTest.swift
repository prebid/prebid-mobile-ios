...
class OXMWebViewTest : XCTestCase, OXMWebViewDelegate {
    ...
    func testRequiredProperties() {
        let webView = OXMWebView(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
    ...
    func testIsVisible() {
        ...
        XCTAssertFalse(OXMWebView.isVisibleView(nil))
        XCTAssertFalse(OXMWebView.isVisibleView(testView))
        XCTAssertFalse(OXMWebView.isVisibleView(parentWindow))
        ...
        XCTAssertTrue(OXMWebView.isVisibleView(testView))
    ...
    func testIsVisible2() {
        ...
        XCTAssertTrue(OXMWebView.isVisibleView(root))
        XCTAssertTrue(OXMWebView.isVisibleView(grandparent))
        XCTAssertTrue(OXMWebView.isVisibleView(parent))
        XCTAssertTrue(OXMWebView.isVisibleView(view))
        ...
        XCTAssertTrue(OXMWebView.isVisibleView(grandparent))
        XCTAssertTrue(OXMWebView.isVisibleView(parent))
        XCTAssertFalse(OXMWebView.isVisibleView(view))
        
        XCTAssertEqual(parent.viewExposure, OXMViewExposure(exposureFactor: 0.2, visibleRectangle: CGRect(x: 0, y: 0, width: 70, height: 20)));
        ...
        XCTAssertTrue(OXMWebView.isVisibleView(root))
        XCTAssertTrue(OXMWebView.isVisibleView(grandparent))
        XCTAssertFalse(OXMWebView.isVisibleView(parent))
        XCTAssertTrue(OXMWebView.isVisibleView(view))
        
        XCTAssertEqual(grandparent.viewExposure, OXMViewExposure(exposureFactor: 0.25, visibleRectangle: CGRect(x: 0, y: 0, width: 50, height: 40)));
        XCTAssertEqual(parent.viewExposure, .zero);
        XCTAssertEqual(view.viewExposure, OXMViewExposure(exposureFactor: 1.0/3, visibleRectangle: CGRect(x: 0, y: 0, width: 30, height: 30)));
    ...
    func testAnyToJSONDict() {
        ...
        //Test valid input
        XCTAssertNotNil(OXMWebView.any(toJSONDict: "{}"))
        XCTAssertNotNil(OXMWebView.any(toJSONDict: "{\"test\" : {\"key\" : \"value\", \"vlue\" : \"key\"}}"))
        
        //Confirm that invalid input will result in a nil
        XCTAssertNil(OXMWebView.any(toJSONDict: nil))
        XCTAssertNil(OXMWebView.any(toJSONDict: UIView()))
        XCTAssertNil(OXMWebView.any(toJSONDict: [""]))
        XCTAssertNil(OXMWebView.any(toJSONDict: ["":""]))
        XCTAssertNil(OXMWebView.any(toJSONDict: ""))
        XCTAssertNil(OXMWebView.any(toJSONDict: "test"))
        XCTAssertNil(OXMWebView.any(toJSONDict: "test{}"))
        XCTAssertNil(OXMWebView.any(toJSONDict: "⛵"))
        XCTAssertNil(OXMWebView.any(toJSONDict: "Ũ"))
        XCTAssertNil(OXMWebView.any(toJSONDict: "[]"))
    }
    
    func testWebViewStateDescription() {
        let cases: [OXMWebViewState] = [.unloaded, .loading,  .loaded]
        
        cases.forEach {
            let description = OXMWebView.webViewStateDescription($0)
    ...
    func testChangeToMRAIDState() {
        let webView = OXMWebView(frame: CGRect(x: 0.0, y: 0.0, width: 100.0, height: 100.0))
        
        XCTAssertEqual(webView.mraidState, .notEnabled)
        webView.changeToMRAIDState(OXMMRAIDState.default)
        XCTAssertEqual(webView.mraidState, OXMMRAIDState.default)
    }
    ...
    func windowOpen(sendTap:Bool, target:String, expectClickThrough:Bool, expectAutoClickSuppressionMessage:Bool) {
        ...
        self.waitForExpectations(timeout: 5) { (error) in
            if error != nil {
                OXMLog.info("sendTap:\(sendTap), target:\"\(target)\", expectClickThrough: \(expectClickThrough), expectAutoClickSuppressionMessage:\(expectAutoClickSuppressionMessage)")
            }
        }
    ...
    func testDecidePolicyForNavigationActionUnprocessingCases(){
        ...
        webView.webView(webView.internalWebView, decidePolicyFor: WKNavigationAction(), decisionHandler: { policy in
            XCTAssertTrue(OXMLog.singleton.getLogFileAsString().contains("No URL found on WKWebView navigation"))
        ...
        webView.webView(webView.internalWebView, decidePolicyFor: navigationAction, decisionHandler: { policy in
            XCTAssertTrue(OXMLog.singleton.getLogFileAsString().contains("Unexpected state "))
            XCTAssertEqual(policy, .cancel)
        })
        ...
        webView.webView(webView.internalWebView, decidePolicyFor: navigationAction, decisionHandler: { policy in
            XCTAssertTrue(OXMLog.singleton.getLogFileAsString().contains("User has not recently tapped."))
            XCTAssertEqual(policy, .cancel)
        })
    }
    ...
    func testExpand() {
        ...
        // Older OSes fire [OXMWebView webView:didFinishNavigation:] earlier
        // than we are expecting.
        let osVersion = Int(UIDevice.current.systemVersion.split(separator: ".")[0])!
        let expectedState = (osVersion >= 10) ? OXMWebViewState.loading.rawValue : OXMWebViewState.loaded.rawValue
    ...
    func testExpandGlobalQueue() {
        ...
        let mockedThread = OXMThread { isCalledFromMainThread in
    ...
    func testMRAID_error() {
        ...
        webView.MRAID_error("test error", action: OXMMRAIDAction.log)
        
        let log = OXMLog.singleton.getLogFileAsString()
        
        XCTAssertTrue(log.contains("Action: [\(OXMMRAIDAction.log.rawValue)] generated error with message [test error]"))
    }
    ...
    private func runTestPrepareForMRAIDWithWindow(coppaValue:Int, coppaFlag:Bool) {
        ...
        let webView = OXMWebView(frame: frame, creativeModel: nil, targeting: targeting)
        webView.delegate = self
        ...
        webView.jsEvaluatingCompletion = { jsCommand, jsRes, error in
            if jsCommand == OXMMRAIDJavascriptCommands.onExposureChange(OXMViewExposure(exposureFactor: 1, visibleRectangle: webView.bounds)) {
    ...
    func testMRAID_onViewableChangeTrue() {
        ...
        let commandBlock = {
            webView.MRAID_onExposureChange(OXMViewExposure(exposureFactor: 1, visibleRectangle: webView.bounds)) // < --- Test target
        }
        
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == OXMMRAIDJavascriptCommands.onViewableChange(true)
        }
    ...
    func testMRAID_onExposureChangeTrue() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == OXMMRAIDJavascriptCommands.onExposureChange(OXMViewExposure(exposureFactor: 1, visibleRectangle: webView.bounds))
        }
    ...
    func testMRAID_updatePlacementType() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == OXMMRAIDJavascriptCommands.updatePlacementType(.inline)
        }
    ...
    func testAudioVolumeChange() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            return jsCommand == OXMMRAIDJavascriptCommands.onAudioVolumeChange(50.0)
        }
    ...
    func testGetCurrentAppOrientation() {
        ...
        let webView = OXMWebView(frame: frame)
        webView.delegate = self
        ...
        webView.jsEvaluatingCompletion = { jsCommand, jsRes, error in
            if jsCommand ==  OXMMRAIDJavascriptCommands.updateCurrentAppOrientation(orientationStr,
                                                                                    locked: false){
                self.expectationCommandExecuted?.fulfill()
            }
        }
    ...
    func testUIApplicationDidChangeStatusBarOrientationNotification() {
        ...
        webView.jsEvaluatingCompletion = { jsCommand, jsRes, error in
            if jsCommand == OXMMRAIDJavascriptCommands.updateMaxSize(OXMFunctions.deviceMaxSize()) {
                self.expectationCommandExecuted?.fulfill();
            }
        }
    ...
    func testLayoutSubviews() {
        ...
        let jsEvaluationBlock: ((String?, Any?, Error?) -> Bool) = { jsCommand, jsRes, error in
            let res = OXMMRAIDJavascriptCommands.updateCurrentPosition(webView.frame)
    ...
    // MARK: - OXMWebViewDelegate
    
    func viewControllerForPresentingModals() -> UIViewController? {
        return UIViewController()
    }
    
    func webViewReady(toDisplay webView: OXMWebView) {
        expectationWebViewReadyToDisplay?.fulfill()
        readyToDisplayBlock?()
    }
    
    func webView(_ webView: OXMWebView, failedToLoadWithError error: Error) {
        expectationWebViewFailed?.fulfill()
    }
    
    func webView(_ webView: OXMWebView, receivedClickthroughLink url: URL) {
        expectationWebViewShouldOpenExternalLink?.fulfill()
    }
    
    func webView(_ webView: OXMWebView, receivedMRAIDLink url: URL) {
        expectationWebViewReceivedMRAIDLink?.fulfill()
    }
    
    // MARK: - Check methods
    
    private func checkJSEvaluating( webView: OXMWebView,
