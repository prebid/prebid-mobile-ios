...
static NSString * const OXMAVPlayerObserverKeyStatus        = @"status";
static NSString * const OXMAVPlayerObserverKeyVolume        = @"volume";
static NSString * const OXMAudioSessionObserverKeyVoulume   = @"outputVolume";

static NSString * const OXMLearnMoreButtonTitle             = @"Learn More";
static NSString * const OXMWatchAgainButtonTitle            = @"Watch Again";
...
@interface OXMVideoView () <UIGestureRecognizerDelegate>

#pragma mark Model

@property (nonatomic, weak) OXMVideoCreative *creative;
@property (nonatomic, strong) OXMEventManager *eventManager;
@property (nonatomic, strong) OXMTouchDownRecognizer *tapdownGestureRecognizer;
@property (nonatomic, strong) OXMLegalButtonDecorator *legalButtonDecorator;
...
- (void)dealloc {
    ...
    if (self.avPlayer) {
        [self.avPlayer.currentItem removeObserver:self forKeyPath:OXMAVPlayerObserverKeyStatus];
        [self.avPlayer removeObserver:self forKeyPath:OXMAVPlayerObserverKeyVolume];
        ...
        [AVAudioSession.sharedInstance removeObserver:self forKeyPath:OXMAudioSessionObserverKeyVoulume];
    }
}

- (void)setupWithEventManager:(OXMEventManager *)eventManager {
    self.vastDurationHasEnded = NO;
    self.showLearnMore = NO;
    self.isPlaybackStarted = NO;
    self.eventManager = eventManager;
    self.accessibilityIdentifier = @"OXMVideoView";
...
- (void)showMediaFileURL:(NSURL *)mediaFileURL preloadedData:(NSData *)preloadedData {
    OXMLogWhereAmI();
    OXMAssert(mediaFileURL && preloadedData);
    if (!(mediaFileURL )) {
        return;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self.avPlayer addObserver:self forKeyPath:OXMAVPlayerObserverKeyVolume options:NSKeyValueObservingOptionNew context:nil];
    });
...
- (void)updateLearnMoreButtonVisibility {
    ...
    OXMCreativeModel *creativeModel = self.creative.creativeModel;
...
- (void)setupObserversWithPlayerItem:(AVPlayerItem *)playerItem {
    ...
    [playerItem addObserver:self forKeyPath:OXMAVPlayerObserverKeyStatus options:NSKeyValueObservingOptionInitial context:nil];
    ...
    [AVAudioSession.sharedInstance addObserver:self forKeyPath:OXMAudioSessionObserverKeyVoulume options:NSKeyValueObservingOptionNew context:nil];
}
...
- (void)updateLearnMoreButton {
    ...
    self.btnLearnMore = [self createButtonWithTitile:OXMLearnMoreButtonTitle action:@selector(btnLearnMoreClick)];
    [self addSubview:self.btnLearnMore];

    [self.btnLearnMore OXMAddBottomRightConstraintsWithMarginSize:CGSizeMake(-25.0, -25.0)];
}
...
- (void)updateProgressBar {
    ...
    OXMCircularProgressBarView *progressBar = [[OXMCircularProgressBarView alloc] initWithFrame:CGRectMake(30,60, 36, 36)];
    ...
    [progressBar OXMAddBottomLeftConstraintsWithViewSize:CGSizeMake(36, 36) marginSize:CGSizeMake(25.0, -25.0)];
...
- (void)updateWatchAgainButton {
    ...
    self.btnWatchAgain = [self createButtonWithTitile:OXMWatchAgainButtonTitle action:@selector(btnWatchAgainClick)];
    [self addSubview:self.btnWatchAgain];
    
    [self.btnWatchAgain OXMAddCropAndCenterConstraintsWithInitialWidth:self.btnWatchAgain.frame.size.width initialHeight:self.btnWatchAgain.frame.size.height];
}
                                 ...
                                 action:(SEL)action
{
    ...
    [button setImage:[UIImage imageNamed:imageName
                                inBundle:[OXMFunctions bundleForSDK]
...
- (void)startPlayback {
    ...
    if (!self.avPlayer) {
        OXMLogError(@"Attempted to display a VideoView with no avPlayer");
        return;
    }
...
- (void)pause {
    if (!self.avPlayer) {
        OXMLogError(@"Attempted to pause a VideoView with no avPlayer");
        return;
    }
    ...
    [self.avPlayer pause];
    [self.eventManager trackEvent:OXMTrackingEventPause];
}
...
- (void)resume {
    ...
    [self.avPlayer play];
    [self.eventManager trackEvent:OXMTrackingEventResume];
}

- (void)stop {
    [self stopWithTrackingEvent:OXMTrackingEventSkip];
}

- (void)stopWithTrackingEvent:(OXMTrackingEvent)trackingEvent {
    if (!self.avPlayer) {
        OXMLogError(@"No AVPlayer to stop");
        return;
    }
...
- (void)addFriendlyObstructionsToMeasurementSession:(OXMOpenMeasurementSession *)session {
    [session addFriendlyObstruction:self.btnLearnMore purpose:OXMOpenMeasurementFriendlyObstructionVideoViewLearnMoreButton];
    [session addFriendlyObstruction:self.progressBar purpose:OXMOpenMeasurementFriendlyObstructionVideoViewProgressBar];
}

#pragma mark - Observers

- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:OXMAVPlayerObserverKeyStatus]) {
        [self onPlayerStatusChanged];
    }
    else if ([keyPath isEqualToString:OXMAVPlayerObserverKeyVolume] && object == self.avPlayer) {
        [self onPlayerVolumeChanged];
    }
    else if ([keyPath isEqualToString:OXMAudioSessionObserverKeyVoulume]) {
...
- (void)completeVideoViewDisplay {
    ...
    if (!self.vastDurationHasEnded) {
        ...
        [self.eventManager trackEvent:OXMTrackingEventComplete];
    }
...
- (CGFloat)handlePeriodicTimeEvent {
    ...
    if (CMTimeCompare(endTime, kCMTimeZero)) {
        ...
        if (self.previousCompletetionPercentage < 0.25 && playbackPercent >= 0.25) {
            [self.eventManager trackEvent:OXMTrackingEventFirstQuartile];
            OXMLogInfo(@"Video Playback Progress: OXMTrackingEventFirstQuartile");
        }

        if (self.previousCompletetionPercentage < 0.50 && playbackPercent >= 0.50) {
            [self.eventManager trackEvent:OXMTrackingEventMidpoint];
            OXMLogInfo(@"Video Playback Progress: OXMTrackingEventMidpoint");
        }
        
        if (self.previousCompletetionPercentage < 0.75 && playbackPercent >= 0.75) {
            [self.eventManager trackEvent:OXMTrackingEventThirdQuartile];
            OXMLogInfo(@"Video Playback Progress: OXMTrackingEventThirdQuartile");
        }
...
- (void)trackStartPlaybackEvents {
    [self.eventManager trackEvent:OXMTrackingEventNormal];
    ...
    OXMLogInfo(@"Video Playback Progress: OXMTrackingEventCreativeView/OXMTrackingEventStart");
}
        ...
        if (self.avPlayer && self.avPlayer.currentItem) {
            switch (self.avPlayer.status) {
                ...
                case AVPlayerStatusUnknown : {
                    OXMLogInfo(@"unknown (This is normal at launch)");
                    return;
                }
                break;
...
- (void)updateLegalButtonDecorator {
    ...
    self.legalButtonDecorator.buttonTouchUpInsideBlock = ^{
        ...
        if (clickthroughBrowserView) {
            ...
            OXMModalManager *modalManager = self.creative.modalManager;            
