...
@interface OXMLocationManager () <CLLocationManagerDelegate>
@property (strong, nullable) CLLocation *location;
@property (strong) id<OXMLocationManagerProtocol> locationManager;
@end

@implementation OXMLocationManager
# pragma mark - Init
+ (nonnull instancetype)singleton {
    static OXMLocationManager *singleton;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        singleton = [[OXMLocationManager alloc] initWithThread:[NSThread currentThread]];
    });
    return singleton;
}
...
- (instancetype)initWithLocationManager:(id<OXMLocationManagerProtocol>)locationManager {
    self = [super init];
    if (!self) {
        return nil;
    }
...
#pragma mark - Private
- (void)setupWithLocationManager:(id <OXMLocationManagerProtocol>)locationManager {
    self.locationManager = locationManager;
    
    self.locationManager.distanceFilter = OXMGeoLocationConstants.DISTANCE_FILTER;
...
- (void)initializeInternalLocationManagerInThread:(id<OXMNSThreadProtocol>)thread {
    ...
    id <OXMLocationManagerProtocol> locationManager = [CLLocationManager<OXMLocationManagerProtocol> new];
    [self setupWithLocationManager:locationManager];
}
