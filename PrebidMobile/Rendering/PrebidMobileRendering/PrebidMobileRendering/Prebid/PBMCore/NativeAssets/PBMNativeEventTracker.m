...
@interface PBMNativeEventTracker ()
@property (nonatomic, strong) NSArray<NSNumber *> *rawMethods;
@end



@implementation PBMNativeEventTracker

- (instancetype)initWithEvent:(PBMNativeEventType)event methods:(NSArray<NSNumber *> *)methods {
    if (!(self = [super init])) {
        return nil;
    }
    _event = event;
...
- (id)copyWithZone:(NSZone *)zone {
    PBMNativeEventTracker * const clone = [[PBMNativeEventTracker alloc] initWithEvent:self.event methods:@[]];
...
- (nullable PBMJsonDictionary *)jsonDictionary {
    PBMMutableJsonDictionary * const result = [[PBMMutableJsonDictionary alloc] init];
