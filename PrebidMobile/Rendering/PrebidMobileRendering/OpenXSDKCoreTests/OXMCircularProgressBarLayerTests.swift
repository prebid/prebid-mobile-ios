//
//  OXMCircularProgressBarLayerTests.swift
//  OpenXSDKCoreTests
//
...
class OXMCircularProgressBarLayerTests: XCTestCase {    
    
    func testNeedsDisplayForKey() {
        XCTAssertTrue(OXMCircularProgressBarLayer.needsDisplay(forKey: "value"))
        XCTAssertFalse(OXMCircularProgressBarLayer.needsDisplay(forKey: "nonNeedsDisplay"))
    }
    
    func testProperties() {
        let circularProgressBarView = OXMCircularProgressBarView(frame: CGRect(x: 0, y: 0, width: 50, height: 50))
        XCTAssertNotNil(circularProgressBarView)
        
        guard let circularProgressBarLayer = circularProgressBarView.layer as? OXMCircularProgressBarLayer else {
            XCTFail()
            return
        }
    ...
    func testUpdateProgress() {
        ...
        let circularProgressBarView = OXMCircularProgressBarView(frame: viewRect)
        XCTAssertNotNil(circularProgressBarView)
