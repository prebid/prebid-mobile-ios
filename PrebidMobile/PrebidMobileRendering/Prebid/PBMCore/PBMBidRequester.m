...
- (void)requestBidsWithCompletion:(void (^)(BidResponse *, NSError *))completion {
    ...
    NSString * const requestServerURL = [PBMHost.shared getHostURLWithHost:self.sdkConfiguration.prebidServerHost error:&hostURLError];
        ...
        if (bidResponse && !trasformationError) {
            ...
            if (tmaxrequest) {
                ...
                const NSTimeInterval remoteTimeout = ([responseDate timeIntervalSinceDate:requestDate]
                                                      + bidResponseTimeout
                                                      + 0.2);
                NSString * const currentServerURL = [PBMHost.shared getHostURLWithHost:self.sdkConfiguration.prebidServerHost error:nil];
