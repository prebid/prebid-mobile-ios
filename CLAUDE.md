...
CI: guards run on every PR (`guards.yml`, ubuntu). Build + quick unit tests run on every
...
- `scripts/testPrebidMobile.sh` installs pods and creates/deletes a named simulator; it
  needs macOS + Xcode. Don't "fix" its SwiftLint tail-comment — SwiftLint enforcement is
