//
//  OXMHTMLCreativeTest_MRAIDResize.swift
//  OpenXSDKCoreTests
//
...
class OXMHTMLCreativeTest_MRAIDResize: OXMHTMLCreativeTest_Base {
    ...
    /**
     ...
     - parameters:
        ...
        - expectedState: If `shouldFulfill`, the `OXMMRAIDState` expected to change to
    */
    func mraidStateChangeExpecation(shouldFulfill: Bool, expectedState: OXMMRAIDState = .default) {
        ...
        self.mockWebView.mock_changeToMRAIDState = { (actualState) in
            if shouldFulfill {
                OXMAssertEq(actualState, expectedState)
            }
            exp.fulfill()
        }
    }
