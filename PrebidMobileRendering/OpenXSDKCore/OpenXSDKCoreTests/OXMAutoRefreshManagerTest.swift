//
//  OXMAutoRefreshManagerTest.swift
//  OpenXSDKCoreTests
//
...
class OXMAutoRefreshManagerTest: XCTestCase {
    func testNoTimerAutoStart() {
        ...
        withExtendedLifetime(OXMAutoRefreshManager(prefetchTime: 0,
                                                   refreshDelay: { 5 },
    ...
    func testNoTimerAutoStartWithPrefetch() {
        let callbackExpectation = expectation(description: "callback called")
        callbackExpectation.isInverted = true
        withExtendedLifetime(OXMAutoRefreshManager(prefetchTime: 2,
                                                   refreshDelay: { 4 },
                                                   mayRefreshNowBlock: { true },
                                                   refreshBlock: callbackExpectation.fulfill)) {
    ...
    func testInitialRequest() {
        let callbackExpectation = expectation(description: "callback called")
        let autoRefreshManager = OXMAutoRefreshManager(prefetchTime: 0,
                                                       refreshDelay: { 5 },
                                                       mayRefreshNowBlock: { true },
                                                       refreshBlock: callbackExpectation.fulfill)
    ...
    func testInitialRequestWithPrefetch() {
        let callbackExpectation = expectation(description: "callback called")
        let autoRefreshManager = OXMAutoRefreshManager(prefetchTime: 2,
                                                       refreshDelay: { 6 },
                                                       mayRefreshNowBlock: { true },
                                                       refreshBlock: callbackExpectation.fulfill)
        let beforeSetup = Date()
    ...
    func testNoRepeatedRequest() {
        ...
        let autoRefreshManager = OXMAutoRefreshManager(prefetchTime: 0, refreshDelay: {3}, mayRefreshNowBlock: {true}) {
    ...
    func testNoRepeatedRequestWithPrefetch() {
        let callbackExpectation = expectation(description: "callback called")
        let nextExpectation = NSMutableArray(object: callbackExpectation)
        let autoRefreshManager = OXMAutoRefreshManager(prefetchTime: 2, refreshDelay: {6}, mayRefreshNowBlock: {true}) {
            (nextExpectation[0] as? XCTestExpectation)?.fulfill()
        }
        autoRefreshManager.setupRefreshTimer()
        waitForExpectations(timeout: 5)
                                         ...
                                         extraTimeToFinish: TimeInterval = 1)
    {
        ...
        let autoRefreshManager = OXMAutoRefreshManager(prefetchTime: prefetchTime,
                                                       refreshDelay: {
                                                       ...
                                                       refreshBlock: {
                                                        ...
                                                        case .refresh(let setupAgain):
                                                            if setupAgain {
                                                                (autoRefreshManagers[0] as? OXMAutoRefreshManager)?
                                                                    .setupRefreshTimer()
                                                            }
                                                        default:
                                                            failIteration()
...
extension OXMAutoRefreshManager {
