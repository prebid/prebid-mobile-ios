//
//  OXABidResponseTransformer+TestExtension.swift
//  OpenXSDKCoreTests
//
...
extension OXABidResponseTransformer {
    static func invalidAccountIDResponse(accountID: String) -> OXMServerResponse {
        return buildResponse("Invalid request: Stored Request with ID=\"\(accountID)\" not found.")
    }
    
    static func invalidConfigIdResponse(configId: String) -> OXMServerResponse {
        return buildResponse("Invalid request: Stored Imp with ID=\"\(configId)\" not found.")
    }
    
    static func invalidSizeResponse(impIndex: Int, formatIndex: Int) -> OXMServerResponse {
    ...
    static var serverErrorResponse: OXMServerResponse {
        return buildResponse("Invalid request: some server reason, probably")
    }
    
    static var nonJsonDicResponse: OXMServerResponse {
        return buildResponse("Some texty (non-JSON-ish) response here")
    }
    
    static var someValidResponse: OXMServerResponse {
        return makeValidResponse(bidPrice: 0.1091000000051168)
    }
    
    static var noWinningBidResponse: OXMServerResponse {
        return makeValidResponse(bidPrice: 0.1091000000051168, noCacheID: true)
    }
    
    static func makeValidResponse(bidPrice: Float, noCacheID: Bool = false) -> OXMServerResponse {
    ...
    static func buildResponse(_ body: String) -> OXMServerResponse {
        let result = OXMServerResponse()
