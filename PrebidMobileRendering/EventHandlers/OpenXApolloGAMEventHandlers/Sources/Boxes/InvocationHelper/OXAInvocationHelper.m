...
NSExceptionName const OXAInvalidInvocationResultTypeException = @"OXAInvalidInvocationResultTypeException";
NSExceptionName const OXAVoidSelectorReturnedValueException = @"OXAVoidSelectorReturnedValueException";

@implementation OXAInvocationHelper
    ...
    return ^(id result) {
        if (exceptionHandlerBlock) {
            NSString *msg = [NSString stringWithFormat:@"Unexpectedly received result (%@ of type %@) for void-result selector",
                             result, [result class]];
            exceptionHandlerBlock([NSException exceptionWithName:OXAVoidSelectorReturnedValueException
                                                          reason:msg
                                                        userInfo:nil]);
        }
    };
    ...
    [self invokeSelector:selector
        ...
        if (![result isKindOfClass:[NSValue class]]) {
            if (exceptionHandlerBlock) {
                ...
                exceptionHandlerBlock([NSException exceptionWithName:OXAInvalidInvocationResultTypeException
                                                              reason:msg
                                                            userInfo:nil]);
            }
            return;
