...
class CustomRendererBannerController: NSObject, AdaptedController, PrebidConfigurableBannerController, BannerViewDelegate {
    ...
    func configurationController() -> BaseConfigurationController? {
        return PrebidBannerConfigurationController(
            controller: self
        )
    }
    ...
    func loadAd() {
        ...
        if adSizes.count > 1 {
            adBannerView?.additionalSizes = Array(
                adSizes.suffix(
                    from: 1
                )
            )
        ...
        if !AppConfiguration.shared.adUnitContextKeywords.isEmpty {
            for keyword in AppConfiguration.shared.adUnitContextKeywords {
                adBannerView?.addContextKeyword(
                    keyword
                )
            }
        }
        ...
        if let appData = AppConfiguration.shared.appContentData {
            ...
            adBannerView?.addAppContentData(
                [ortbAppContentData]
            )
        }
    ...
    func bannerView(
        ...
        rootController?.bannerView.constraints.first {
    ...
    func bannerViewWillPresentModal(
        _ bannerView: BannerView
    ) {
        adViewWillPresentScreenButton.isEnabled = true
    }
    
    func bannerViewDidDismissModal(
        _ bannerView: BannerView
    ) {
    ...
    private func setupAdapterController() {
        ...
        rootController?.actionsView.addArrangedSubview(
            configIdLabel
        )
        lastLoadedAdSizeLabel.isHidden = true
        rootController?.actionsView.addArrangedSubview(
            lastLoadedAdSizeLabel
        )
    }
    
    private func setupActions() {
        rootController?.setupAction(
            adViewDidReceiveAdButton,
            "adViewDidReceiveAd called",
            accessibilityLabel: "adViewDidReceiveAd called"
        )
        rootController?.setupAction(
            adViewDidFailToLoadAdButton,
            "adViewDidFailToLoadAd called"
        )
        rootController?.setupAction(
            adViewWillPresentScreenButton,
            "adViewWillPresentScreen called"
        )
        rootController?.setupAction(
            adViewDidDismissScreenButton,
            "adViewDidDismissScreen called"
        )
        rootController?.setupAction(
            adViewWillLeaveApplicationButton,
            "adViewWillLeaveApplication called"
        )
