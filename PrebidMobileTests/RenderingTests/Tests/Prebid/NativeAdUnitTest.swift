...
class NativeAdUnitTest: XCTestCase, WinningBidResponseFabricator {
    ...
    func testDesignatedInit_noBlockCalled() {
        ...
        nativeAdConfig.eventtrackers = [PBRNativeEventTracker(event: NativeEventType.omid.rawValue,
...
{"link": {"url": "\(someLinkUrl)"}}
"""
        let expectedNativeAd = PBRNativeAd(nativeAdMarkup: try! PBMNativeAdMarkup(jsonString: expectedAdMarkup))
