...
class AdUnitTests: XCTestCase {
    ...
    func testFetchDemandWithRequest() {
        //given
        let expectation = expectation(description: "\(#function)")
        var bidInfo: BidInfo?
        
        let expectedResult = ResultCode.prebidDemandFetchSuccess
        let expectedTargetingKeywords = ["key1": "value1"]
        let expectedExp = 5.0
        
        let adUnit = AdUnit(configId: "138c4d03-0efb-4498-9dc6-cb5a9acb2ea4", size: CGSize.zero, adFormats: [])
        ...
        AdUnitSwizzleHelper.testScenario = expectedResult
        AdUnitSwizzleHelper.targetingKeywords = expectedTargetingKeywords
        AdUnitSwizzleHelper.exp = expectedExp
        
        AdUnitSwizzleHelper.toggleFetchDemand()
        
        //when
        adUnit.fetchDemand(request: request) { info in
            bidInfo = info
            expectation.fulfill()
        }
        ...
        //then
        XCTAssertEqual(expectedResult, bidInfo?.result)
        XCTAssertEqual(expectedTargetingKeywords, bidInfo?.targetingKeywords)
        XCTAssertEqual(expectedExp, bidInfo?.exp)
    }
    
    func testFetchDemandWithAdObjectAndRequest() {
        //given
        let expectation = expectation(description: "\(#function)")
        var resultCode: ResultCode?
        let testObject: AnyObject = () as AnyObject
        ...
        //when
        adUnit.fetchDemand(adObject: testObject, request: request) { result in
            resultCode = result
            expectation.fulfill()
        }
    ...
    func testAdUnitConfiguration_banner() {
        ...
        var adUnit = AdUnit(configId: "138c4d03-0efb-4498-9dc6-cb5a9acb2ea4", size: CGSize.zero, adFormats: [])
        ...
        adUnit.fetchDemand(request: request) { _ in
            XCTAssertNotNil(adUnit.adUnitConfig.adConfiguration.bannerParameters)
            XCTAssertEqual(adUnit.adUnitConfig.adFormats, [.banner])
            XCTAssertEqual(adUnit.adUnitConfig.adSize, CGSize(width: 300, height: 250))
            XCTAssertEqual(adUnit.adUnitConfig.additionalSizes, [CGSize(width: 320, height: 50)])
            expectation.fulfill()
        }
        
        adUnit = AdUnit(configId: "138c4d03-0efb-4498-9dc6-cb5a9acb2ea4", size: CGSize.zero, adFormats: [])
    ...
    func testAdUnitConfiguration_video() {
        ...
        // fetchDemand(request:completion)
        adUnit.fetchDemand(request: request) { _ in
            XCTAssertNotNil(adUnit.adUnitConfig.adConfiguration.videoParameters)
            XCTAssertEqual(adUnit.adUnitConfig.adSize, CGSize(width: 300, height: 250))
            XCTAssertEqual(adUnit.adUnitConfig.adFormats, [.video])
            expectation.fulfill()
        }
    ...
    func testAdUnitConfiguration_native() {
        ...
        // fetchDemand(request:completion)
        adUnit.fetchDemand(request: request) { _ in
            XCTAssertNotNil(adUnit.adUnitConfig.nativeAdConfiguration)
            XCTAssertEqual(adUnit.adUnitConfig.nativeAdConfiguration?.markupRequestObject.assets, [title, body, cta])
            XCTAssertEqual(adUnit.adUnitConfig.adFormats, [.native])
            expectation.fulfill()
        }
    ...
    func testAdUnitConfiguration_interstitial() {
        ...
        adUnit.fetchDemand(request: request) { _ in
            ...
            XCTAssertEqual(adUnit.adUnitConfig.adPosition, .fullScreen)
            XCTAssertEqual(adUnit.adUnitConfig.adConfiguration.videoParameters.placement, .Interstitial)
            XCTAssertEqual(adUnit.adUnitConfig.minSizePerc, NSValue(cgSize: CGSize(width: 4, height: 4)))
            expectation.fulfill()
        }
    ...
    func testAdUnitConfiguration_interstitial_rewarded() {
        ...
        adUnit.fetchDemand(request: request) { _ in
            XCTAssertTrue(adUnit.adUnitConfig.adConfiguration.isInterstitialAd)
            XCTAssertTrue(adUnit.adUnitConfig.adConfiguration.isOptIn)
            XCTAssertEqual(adUnit.adUnitConfig.adPosition, .fullScreen)
            XCTAssertEqual(adUnit.adUnitConfig.adConfiguration.videoParameters.placement, .Interstitial)
    ...
    func testAdUnitConfiguration_extData() {
        ...
        // fetchDemand(request:completion)
        adUnit.fetchDemand(request: request) { _ in
            XCTAssertEqual(adUnit.adUnitConfig.getExtData(), ["key1": ["value1"]])
            expectation.fulfill()
        }
    ...
    func testAdUnitConfiguration_appContent() {
        ...
        // fetchDemand(request:completion)
        adUnit.fetchDemand(request: request) { _ in
            let realAppContent = adUnit.adUnitConfig.getAppContent()
    ...
    func testAdUnitConfiguration_userData() {
        ...
        // fetchDemand(request:completion)
        adUnit.fetchDemand(request: request) { _ in
            let realUserData = adUnit.adUnitConfig.getUserData()
    ...
    func testAdUnitConfiguration_extKeywords() {
        ...
        // fetchDemand(request:completion)
        adUnit.fetchDemand(request: request) { _ in
            XCTAssertEqual(adUnit.adUnitConfig.getExtKeywords(), ["test1"])
            expectation.fulfill()
        }
